/**
 * 2.2.2
 * Hello pop | Developed by Amatris
...
var Hello = (function () {
    ...
    function Hello(conf = {}) {
        ...
        this.replaceTemplate = (text) => {
            ...
            return replaceArray(
                text,
                ["{text}", "{pop_type}", "{pop_mode}", "{pop_theme}", "{type}", "{attr}", "{id}", "{content}", "{title}", "{icon}", "{loader}"],
                [_this.scope.text, _this.conf.type, _this.conf.mode, "theme-" + _this.conf.theme, _this.scope.type, _this.scope.attr, _this.scope.id, _this.conf.text, _this.conf.title, iconHTML, _this.templates.loader]
            );
        }
    ...
    <div class="hello-pop {pop_type} {pop_theme} {pop_mode}">
        {loader}
        ...
        <div class="hello-footer">{footer}</div>
            ...
            toast: `<div class="hello-toast {pop_mode}" data-hello="{id}" dir="auto">{content}</div>`
        };

        this.set = (conf) => this.conf = Object.assign(this.default_conf, conf);

        this.build = () => {
            if(_this.conf.singleMode) Hello.closeAll();
        ...
        this.destroy = () => {
            if (_this.$current === null) return null;
            if(typeof _this.conf.onDelete === "function")
                _this.conf.onDelete();
    ...
    Hello.makeToast = (text, timeout=null) => {
        let hello = new Hello(Object.assign({
            type: "toast",
            text: text,
            timeout: timeout
